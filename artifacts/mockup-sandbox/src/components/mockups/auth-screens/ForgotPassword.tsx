    ...
    <div className="min-h-screen bg-[#08080e] flex flex-col p-6 font-['Inter']">
      ...
      {!sent ? (
        <>
          ...
          <button className="w-full bg-white/5 border border-white/10 rounded-xl py-3.5 text-sm text-white/60 font-medium">
            Sign in with Google
          </button>
        </>
      ) : (
        ...
        <div className="flex flex-col items-center text-center flex-1 justify-center gap-5">
          ...
          <button className="w-full bg-[#a855f7] text-white font-bold rounded-xl py-4">
            Open Mail App
          </button>
        </div>
      )}

      <div className="w-full bg-[#a855f7]/5 border border-[#a855f7]/20 rounded-xl p-3 text-[10px] text-white/40 leading-relaxed mt-6">
        <span className="text-[#a855f7] font-bold">UX:</span> Two states — request form + confirmation. Countdown timer on resend prevents spam anxiety. "Open Mail App" = instant next action. No dead end after submission.
      </div>
    </div>
  );
}
