import { Shield, Check, X, ChevronDown, ChevronUp, Play, Pause, Filter, Users, TrendingUp, BarChart2, Clock } from "lucide-react";
...
export function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#08080e] flex flex-col font-['Inter']">
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-10 pt-3 space-y-3">
          ...
          <>
            ...
            <div className="flex gap-1.5">
              {[["pending","yellow"],["approved","green"],["rejected","red"]].map(([f,c]) => (
                <button key={f} onClick={() => setFilter(f as any)} className={`flex-1 py-1.5 rounded-lg text-[10px] font-bold uppercase transition-all ${filter === f ? `bg-${c}-500/20 text-${c}-400 border border-${c}-500/30` : "text-white/30 bg-white/5"}`}>
                  {f}
                </button>
              ))}
            </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex gap-1 mt-0.5 flex-wrap">
                      {song.tags.slice(0,3).map(t => (
                        <span key={t} className="px-1.5 py-0 rounded-full bg-[#a855f7]/10 text-[8px] text-[#a855f7] border border-[#a855f7]/15 leading-4">{t}</span>
                      ))}
                    </div>
                  </div>
                ...
                {expanded === song.id && (
                  <div className="border-t border-white/5 px-3 pb-3 pt-3 space-y-3">
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => approve(song.id)} className="flex-1 py-3 bg-green-500/20 text-green-400 border border-green-500/30 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 hover:bg-green-500/30 active:scale-[0.97] transition-all">
                        <Check size={13} /> Approve
                      </button>
                      <button onClick={() => reject(song.id)} className="flex-1 py-3 bg-red-500/10 text-red-400 border border-red-500/20 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 hover:bg-red-500/20 active:scale-[0.97] transition-all">
                        <X size={13} /> Reject
                      </button>
                    </div>
                  </div>
      ...
      <div className="mx-4 mb-3 bg-[#a855f7]/5 border border-[#a855f7]/20 rounded-xl p-2.5 text-[9px] text-white/40 leading-relaxed">
        <span className="text-[#a855f7] font-bold">UX:</span> Expand-on-tap pattern = no clutter. Audio preview = informed decision. Status badge counter in header = urgency without opening. Approve/reject glow on action = immediate feedback. Green/red semantic colors = zero learning curve.
      </div>
    </div>
  );
}
